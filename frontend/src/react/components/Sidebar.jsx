            ...
            <div className="col-2 mt-4">
                <ul className="list-group folders">
                    { this.props.folders.map((folder, i) =>
                      <Folder key = {i} folder = { folder } toggleUpdateModal={this.toggleUpdateModal} toggleDeleteModal={this.toggleDeleteModal} toggleCannotDeleteModal={this.toggleCannotDeleteModal} />
                    )}
                    <ModalNewFolder/>
                </ul>
...
function Folder (props) {
  ...
  return (
    <li className={ "list-group-item list-group-item-action " +  isActive } onClick={() => alert("opens folder")}>
        ...
        &nbsp;({props.folder.count})
        <div>
          <i className="fa fa-trash float-right folder-actions" aria-hidden="true" data-id={props.folder._id} data-name={props.folder.name} onClick={props.folder.count ? props.toggleCannotDeleteModal : props.toggleDeleteModal} ></i>
